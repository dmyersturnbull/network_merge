...
public class BronKerboschMergeManagerTest {
	...
	public void testMultiedged() {
		...
		CleverGraph graph = GraphMLAdaptor.readGraph(interactionInput, homologyInput);
		assertEquals(3, graph.getVertexCount()); // just a sanity check for the test itself
		assertEquals(2, graph.getHomologyCount()); // just a sanity check for the test itself
		assertEquals(2, graph.getInteractionCount()); // just a sanity check for the test itself
		BronKerboschMergeManager merge = new BronKerboschMergeManager();
		merge.merge(graph);
